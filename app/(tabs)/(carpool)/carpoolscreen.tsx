...
export default function CarpoolHome() {
  ...
  const [cards, setCards] = useState<Carpool[]>([]);
  const [loading, setLoading] = useState(true);

  const addItem = (newCard: Carpool) => {
    setCards(prevItems => [...prevItems, newCard]);
  };
  ...
  const getCardsByAuthor = async () => {
    try {
      ...
      const carpoolPromises: Promise<void>[] = [];

      qs.forEach(userDoc => {
        const userData = userDoc.data() as { carpools: string[] };
        userData.carpools.forEach(carpoolId => {
          const carpoolRef = doc(db, 'carpools', carpoolId);
          carpoolPromises.push(
            getDoc(carpoolRef).then(carpoolSnap => {
              if (carpoolSnap.exists()) {
                const carpoolData = carpoolSnap.data() as Carpool;
                addItem({...carpoolData });
              } else {
    ...
    <View style={{ flex: 1, paddingTop: 100, justifyContent: "flex-start" }}>
      <View style={customStyles.container}>
        <IconButton
          ...
          press={() => { router.navigate("/(tabs)/(carpool)/createcarpool") }}
        />
        <IconButton
          size={28}
          icon={<FontAwesome5 name="search" color={"white"} size={14} />}
          press={() => { router.navigate("/(tabs)/(carpool)/findcarpool") }}
        />
        <IconButton
          size={28}
          icon={<FontAwesome5 name="sync" color={"white"} size={14} />}
          press={() => { getCardsByAuthor(); }}
        />
      </View>
